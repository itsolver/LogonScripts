...
1. Ensure you have these PowerShell scripts in your repository:
   a. `install-user-logon-script.ps1`: Deployed via Intune.
   b. `UserLogonScript.ps1`: Main script that runs for all users at logon.

2. `UserLogonScript.ps1` is stored in a publicly accessible GitHub location.

3. Locate `install-user-logon-script.ps1` at:
   `C:\Users\itsol\Projects\itsolver\LogonScripts\BayBreezeTaxAccountants\UserLogonScript\src\install-user-logon-script.ps1`

4. Create a Win32 app using IntuneWinAppUtil:
   - Download IntuneWinAppUtil from Microsoft's GitHub.
   - Place `install-user-logon-script.ps1` in a folder.
   - Run:
     ```
     IntuneWinAppUtil.exe -c C:\Users\itsol\Projects\itsolver\LogonScripts\BayBreezeTaxAccountants\UserLogonScript\src -s install-user-logon-script.ps1 -o C:\Users\itsol\Projects\itsolver\LogonScripts\BayBreezeTaxAccountants\UserLogonScript
     ```

5. Create a new Win32 app in Intune:
   - Upload the .intunewin file.
   - Configure:
     - Install command: `powershell.exe -executionpolicy bypass -file install-user-logon-script.ps1`
...
1. Intune-deployed Win32 app runs `install-user-logon-script.ps1` in system context:
   - Downloads `install-user-logon-script.ps1` locally
   - Creates a scheduled task for user logon
   - Attempts immediate script execution

2. Scheduled task runs `install-user-logon-script.ps1` for all users at subsequent logons.

3. Update `install-user-logon-script.ps1` in your repository without Intune redeployment.
...
- Regularly update `install-user-logon-script.ps1` for security and functionality.
